...
const std::string Shader::Init(const char *vertexSource,
                               const char *fragmentSource, const char *fileName)
{
  // Creates a  vertex shader object - shader type
  GLuint vertexShader = glCreateShader(GL_VERTEX_SHADER);

  // Replaces the source code in a shader object - shader, count,
  ...
  // Creates a program object, A program object is an object to which shader
...
{
  glUniform4f(glGetUniformLocation(ID, uniform), unit.x, unit.y, unit.z,
              unit.w);
}
...
// TODO: return 0 if not compiled, if shader class set to empty shader if 0 and
// let user know of the error
const std::string Shader::CompileErrors(unsigned int shader, const char *type,
